...
function AuthCallbackContent() {
  ...
  const runCodeExchange = useCallback(async () => {
    ...
    if (result.success && result.user) {
      ...
      const safe = (typeof returnTo === 'string' && returnTo.startsWith('/') && !returnTo.startsWith('//')) ? returnTo : '/dashboard'
      router.push(safe)
    } else {
  ...
  return (
    <LoadingOverlay logoSrc="/drop_icon_no_margins.png" title="Drop" portal={false} />
  )
}

export default function AuthCallback() {
  return (
    <Suspense fallback={<LoadingOverlay logoSrc="/drop_icon_no_margins.png" title="Drop" portal={false} />}>
      <AuthCallbackContent />
    </Suspense>
  )
}
